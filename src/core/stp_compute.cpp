...
Eigen::MatrixXi stpm_basic_product(Eigen::MatrixXi matrix_f, Eigen::MatrixXi matrix_b)
{
  ...
  else
  {
    ...
    { c=n_col%p_row; n_col=p_row;  p_row=c;}
    z=m*n/n_col;
...
Eigen::MatrixXi normalize_matrix(std::vector<Eigen::MatrixXi> matrix_chain)
{
  ...
  {
    if(matrix_chain[i](0,0)>max) max=matrix_chain[i](0,0);
  }
    ...
    if(matrix_chain[p_variable].rows() == 2 && matrix_chain[p_variable].cols() == 1)  //1:find a variable
    {
      ...
      else //idx[matrix_chain[p_variable](0,0)]!=0  false
      {
        ...
        else 
        {
          ...
          for(int i=1;i<=max;i++)
          {
            if(idx[i]!=0&&idx[i]>idx[matrix_chain[p_variable](0,0)]) idx[i]--;
          }
      ...
      while(p > p_variable+1) { matrix_chain.pop_back(); p--;}  //?
      ...
      if(flag) matrix_chain.push_back(Mr);
      continue;
    }
    else { p_variable--;}
  }
...
{
  Eigen::MatrixXi m(2,2);   // MatrixXd 表示的是动态数组，初始化的时候指定数组的行数和列数
  m(0,0) = 1;               //m(i,j) 表示第i行第j列的值，这里对数组进行初始化
  m(0,1) = 2;
  m(1,0) = 3;
  m(1,1) = 1;
  std::cout << "stp_kron_product front matrix:" << std::endl<<std::endl;
  std::cout << m << std::endl<< std::endl;     // eigen重载了<<运算符，可以直接输出eigen矩阵的值
  Eigen::MatrixXi x;
  Eigen::MatrixXi n(2,2);   // MatrixXd 表示的是动态数组，初始化的时候指定数组的行数和列数
  n(0,0) = 0;               //m(i,j) 表示第i行第j列的值，这里对数组进行初始化
  n(0,1) = 3;
  n(1,0) = 2;
  n(1,1) = 1;
  std::cout << "stp_kron_product back matrix:" << std::endl << std::endl;
  std::cout << n << std::endl<< std::endl;
  x=stp_kron_product(m,n);
  std::cout << "stp_kron_product:" << std::endl << std::endl;
...
void test_stpm_basic_product()  //p35 example
{
    ...
    m3<<1,2,-1,2,0,1,
        2,3,3,3,1,1,
        1,2,3,4,5,6;
    std::cout<<"The (left) semi-tensor product of m1 and m2:"<<std::endl<<std::endl;
    std::cout<<"m1:"<<std::endl<<m1<<std::endl<<std::endl;
    std::cout<<"m2:"<<std::endl<<m2<<std::endl<<std::endl;

    std::cout<<"use stpm_basic_product:"<<std::endl<<std::endl;
    std::cout<<stpm_basic_product(m1,m2)<<std::endl<<std::endl;
    std::cout<<"use stpm_basic_product_boost:"<<std::endl<<std::endl;
    std::cout<<stpm_basic_product_boost(m1,m2)<<std::endl<<std::endl;

    std::cout<<"The (left) semi-tensor product of m2 and m3:"<<std::endl<<std::endl;
    std::cout<<"m2:"<<std::endl<<m2<<std::endl<<std::endl;
    std::cout<<"m3:"<<std::endl<<m3<<std::endl<<std::endl;    
    std::cout<<"use stpm_basic_product:"<<std::endl<<std::endl;
    std::cout<<stpm_basic_product(m2,m3)<<std::endl<<std::endl;
    std::cout<<"use stpm_basic_product_boost:"<<std::endl<<std::endl;
    std::cout<<stpm_basic_product_boost(m2,m3)<<std::endl<<std::endl;
}
void test_swap_matrix()
{
  std::cout<<"swap_matrix(2,4):"<<std::endl<<std::endl;
  std::cout<<swap_matrix(2,4)<<std::endl<<std::endl;
  std::cout<<"swap_matrix(3,2):"<<std::endl<<std::endl;
  std::cout<<swap_matrix(3,2)<<std::endl<<std::endl;
  std::cout<<"swap_matrix(3,3):"<<std::endl<<std::endl;
  std::cout<<swap_matrix(3,3)<<std::endl<<std::endl;
}
void test_matrix_chain_multiplication()
{
  ...
  matrix_chain.push_back(Mn);
  matrix_chain.push_back(B);
  std::cout<<"the matrix_chain from (a ↔ ¬b) ∧ (b ↔ ¬c) ∧ (c ↔ ¬a ∧ ¬b) , when a=0,b=1 and c=0 it is ture.(it is a case in book)"<<std::endl<<std::endl;
...
void test1_normalize_matrix() 
{
    ...
    matrix_chain.push_back(Mn);
    matrix_chain.push_back(B);
    std::cout<<"the matrix_chain from (a ↔ ¬b) ∧ (b ↔ ¬c) ∧ (c ↔ ¬a ∧ ¬b)"<<std::endl<<std::endl;
    ...
    matrix_chain.push_back(f);
    std::cout<<"a=0,b=1,c=0"<<std::endl<<std::endl;
    std::cout<<matrix_chain_multiplication(matrix_chain)<<std::endl;
}
void test2_normalize_matrix() 
{
  ...
  Eigen::MatrixXi matrix=normalize_matrix(matrix_chain);
  std::cout<<std::endl<<"normalize matrix:"<<std::endl<<matrix<<std::endl<<std::endl;
  std::cout<<std::endl<<"Then we assign values to all variables by some solution."<<std::endl<<std::endl;
  for(int i=0;i<solutions.size();i++)
  {
    ...
    for(int j=0;j<solutions[i].size();j++)
    {
      ...
      if(solutions[i][j]=='1')  matrix_chain.push_back(t);
      else matrix_chain.push_back(f);
    }
    if(i!=solutions.size()-1) std::cout<<std::endl<<"SAT verify:"<<std::endl<<matrix_chain_multiplication(matrix_chain)<<std::endl<<std::endl;
    else std::cout<<std::endl<<"UNSAT verify:"<<std::endl<<matrix_chain_multiplication(matrix_chain)<<std::endl<<std::endl;
  }
} 
void test3_normalize_matrix() 
{
  ...
  std::cout<<"1 2 0"<<std::endl;
  std::cout<<"-1 -2 0"<<std::endl;
  std::cout<<"1 -2 0"<<std::endl<<std::endl;
